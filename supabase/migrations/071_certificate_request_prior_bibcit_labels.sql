-- Prior BibCit wording for certificate request feed + notifications.

update public.community_activity
set title = regexp_replace(title, 'submitted a certificate request', 'confirmed prior BibCit', 'i')
where feed_category = 'certificate_request'
  and title ~* 'submitted a certificate request';

update public.notification_events
set
  title = regexp_replace(title, 'submitted a certificate request', 'confirmed prior BibCit', 'i'),
  body = case
    when body is null then null
    else regexp_replace(body, 'submitted (an external )?certificate request', 'confirmed prior BibCit', 'i')
  end
where title ~* 'certificate request'
   or body ~* 'submitted (an external )?certificate request';

-- Keep trigger fallback aligned when payload.title is missing.
...
begin
  ...
  if ntitle is null then
    ntitle := case new.action
      ...
      when 'certificate_request_submitted' then 'Prior BibCit'
  ...
  if nbody is null then
    case new.action
      ...
      when 'certificate_request_submitted' then
        nbody := coalesce(
          nullif(trim(p->>'text'), ''),
          'A member confirmed prior BibCit.'
        );
      else
        nbody := null;
    end case;
