-- New member registration copy for Community in Action + notification bell.
...
begin
  ...
  fn := nullif(trim(coalesce(new.first_name, '')), '');
  ln := nullif(trim(coalesce(new.last_name, '')), '');
  if fn is not null and ln is not null and length(ln) > 0 then
    ...
    short_name := upper(left(ln, 1)) || '.';
  else
    short_name := coalesce(nullif(trim(coalesce(new.display_name, '')), ''), 'A member');
  end if;
  ...
  values (
    'member',
    '🎉 ' || short_name || ' joined FlashPoint Army!',
    'Welcome to the movement. Start your journey today!',
    nullif(st, ''),
    'celebration',
    new.id
  );
    ...
    case new.action
      ...
      when 'user_registered' then
        ...
        if reg_fn is not null and reg_ln is not null and length(reg_ln) > 0 then
          ...
          short_name := upper(left(reg_ln, 1)) || '.';
        else
          short_name := 'A member';
        end if;
        ntitle := '🎉 ' || short_name || ' joined FlashPoint Army!';
        nbody := 'Welcome to the movement. Start your journey today!';
