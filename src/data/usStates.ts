/** US state for searchable selects: FIPS state id + USPS abbreviation + name. */
export type USStateOption = {
  /** USPS 2-letter code (stored in `chapters.state`). */
  code: string;
  /** ANSI/FIPS state code (string, e.g. "48" for Texas). Used as Autocomplete value id. */
  id: string;
  name: string;
};

/** 50 states + DC; FIPS codes per ANSI INCITS 38. */
...
const byId = new Map(US_STATES.map((s) => [s.id, s]));

export function usStateByCode(code: string | null | undefined): USStateOption | null {
  if (!code) return null;
  return byCode.get(code.trim().toUpperCase()) ?? null;
}

export function usStateById(id: string | null | undefined): USStateOption | null {
  if (!id) return null;
  return byId.get(id.trim()) ?? null;
}

export function filterUsStatesByQuery(options: USStateOption[], query: string): USStateOption[] {
  const q = query.trim().toLowerCase();
  if (!q) return options;
  return options.filter((o) => {
    const blob = `${o.name} ${o.code} ${o.id}`.toLowerCase();
