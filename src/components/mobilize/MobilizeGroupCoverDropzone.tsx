...
export default function MobilizeGroupCoverDropzone({ value, onChange, disabled = false }: Props) {
  ...
  const previewSrc = value.trim() ? publicAssetSrc(value.trim()) : "";
      ...
      {previewSrc ? (
        ...
        >
          <Box
            ...
            src={previewSrc}
            alt=""
            sx={{ width: "100%", maxHeight: 200, objectFit: "cover", display: "block", bgcolor: "rgba(0,0,0,0.3)" }}
          />
          <Stack direction="row" spacing={1} sx={{ p: 1, bgcolor: "rgba(0,0,0,0.55)" }}>
            <Button
              size="small"
              variant="outlined"
              startIcon={<ImageIcon />}
