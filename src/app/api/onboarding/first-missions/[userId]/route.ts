...
const STATUSES: FirstMissionStepStatus[] = ["locked", "pending", "in_progress", "completed"];
...
export async function PATCH(req: Request, ctx: { params: Promise<{ userId: string }> }) {
  ...
  if (body.tutor_id != null && body.tutor_id !== "" && !UUID_RE.test(body.tutor_id)) {
    return NextResponse.json({ error: "Invalid coach/tutor id." }, { status: 400 });
  }
