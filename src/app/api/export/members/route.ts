...
export async function GET(req: Request) {
  ...
  try {
    ...
    const rows = await buildUserDirectoryExportRows(admin, "members", { chapterId, stateFilter });
    const dateStamp = new Date().toISOString().slice(0, 10);
    const filename = `members-${dateStamp}.xlsx`;
    const buffer = buildXlsxBuffer(rows, "Members");
