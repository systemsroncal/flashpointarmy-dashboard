...
export default async function FirstMissionsPageContent() {
  ...
  if (!elevated || !can(permissions, MODULE_SLUGS.courses, "read")) {
    return (
      <Paper sx={{ p: 3, bgcolor: "rgba(0,0,0,0.45)" }}>
        <Typography color="error">You do not have access to first mission records.</Typography>
      </Paper>
    );
  }
  ...
  return <FirstMissionsAdminClient chapterOptions={chapterOptions} />;
}
